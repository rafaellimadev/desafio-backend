...
class Flexi_auth extends Flexi_auth_lite
{
	...
	{
		$retorno = $this->CI->flexi_auth_model->login($identity, $password, $remember_user);
		if ($retorno->success)
		{
			return $retorno;
		}
		...
		if (! $retorno->message)
		{
			$retorno->message = "Não foi possível realizar o login! Verifique se todos os dados foram preenchidos corretamente ou tente mais tarde. Se mesmo assim não for possível acessar, entre em contato conosco no email contato@spinfast.org para que possamos lhe ajudar";
		}
	...
	public function forgotten_password($identifier) 
	{
		...
		{
			$retorno->message = "E-mail informado não encontrado! Verifique se foi preenchido corretamente.";
			return $retorno;
		}
		
		$retorno = $this->CI->flexi_auth_model->forgotten_password($identity);
		if ($retorno->success)
		{
			// Get user information.
			$sql_select = array(
			...
			//enviar_emails($email_to, "Recuperação de Senha","email/padrao", array("dados" => $dados));
			
			$dados = array();
			$dados['content'] = 'email/esqueci_senha';
			$dados['titulo']  = 'Recuperação de Senha';
			$dados['link']    =  base_url()."index.php/auth/manual_reset_forgotten_password/{$user_id}/{$forgotten_password_token}";
		
			if (enviar_emails($email_to,"Recuperação de Senha","email/padrao", array("dados" => $dados)))
			{
		...
		$retorno->message = "Não foi possível recuperar a senha! Tente novamente mais tarde.";
		return $retorno;
	}
	...
	public function forgotten_password_complete($user_id, $forgot_password_token, $new_password = FALSE, $send_email = FALSE)
	{
		...
		if ($this->CI->flexi_auth_model->validate_forgotten_password_token($user_id, $forgot_password_token))
		{
			...
			if (!is_object($user))
			{
				$retorno->message = "Não foi possível alterar a senha! Tente novamente mais tarde";
				return $retorno;
			}
			...
			{
				$this->login($identity, $new_password);
				$retorno->success = true;
	 ...
	 * @author Rob Hussey
	 */
	public function insert_user($email, $username = FALSE, $password, $user_data, $group_id = FALSE, $activate = FALSE,$indicador_id = FALSE,$bloco_id = FALSE) 
	{
		$retorno = $this->CI->flexi_auth_model->insert_user($email, $username, $password, $user_data, $group_id, $indicador_id, $bloco_id);
		...
		if ($retorno->success)
		{
			...
			if ($activate)
			{
				...
				$this->CI->flexi_auth_model->activate_user($retorno->user_id, FALSE, FALSE, $clear_token);		
			}
			...
			$sql_where[$this->CI->auth->tbl_col_user_account['id']] = $retorno->user_id;
			...
			{
				$retorno->success = false;
				$retorno->message = "Não foi possível cadastrar o usuário";
				return $retorno;
			}
			...
			if (!empty($activation_token))
			{
				...
				{
					$this->CI->flexi_auth_model->set_status_message('activation_email_successful', 'config');
					return $retorno->user_id;
				}

				$retorno->success = false;
				$retorno->message = "Não foi possível ativar o usuário";
				return $retorno;
			}
